...
namespace YandexMobileAds.Platforms.iOS
{
    ...
    public class BannerClient : IBannerClient, IDisposable
    {
        ...
        public BannerClient(string blockId, BannerAdSize adSize, AdPosition position)
        {
            this._selfPointer = GCHandle.ToIntPtr(GCHandle.Alloc(this));

            BannerAdSizeClient adSizeClient = new BannerAdSizeClient(adSize);
        ...
        {
            AdRequestClient adRequestClient = null;
            if (adRequest != null)
            {
                adRequestClient = new AdRequestClient(adRequest);
            }
            BannerBridge.YMAUnityLoadBannerView(
                this.ObjectId,
                adRequestClient.ObjectId
            );
        }
