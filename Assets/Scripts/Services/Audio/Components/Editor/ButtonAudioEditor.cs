...
namespace Services.Audio.Components.Editor
{
    ...
    public class ButtonAudioEditor : UnityEditor.Editor
    {
        ...
        {
            _buttonAudio = target as ButtonAudio;
        ...
        private void DrawAudioHelper()
        {
            ...
            var componentKey = _buttonAudio.SoundKey;
            if (string.IsNullOrEmpty(componentKey)) return;
            
            if (GUI.changed)
            {
                _findKeys.Clear();
                EditorUtility.SetDirty(_buttonAudio);
            }
                ...
                foreach (var entryKey in _entryKeys)
                {
                    ...
                    if (hasParameter)
                    {
                        ...
                        if (GUILayout.Button(entryKey, "CN CountBadge"))
                        {
                            ...
                            _buttonAudio.SoundKey = entryKey;
                            EditorUtility.SetDirty(_buttonAudio);
                        }
